...
import slice from '../Reduxcomponents/slice';
    ...
    .then(obj => {
      if(Object.keys(obj).length===0)
      {
        ...
        if(obj.status === false)
        {
          ...
          if(obj.typeOfUser === 1)
          {
            localStorage.setItem("logincus",JSON.stringify(obj));
              navigate("/CustomerHomePage");

          }
          else if(obj.typeOfUser === 2)
          {
            localStorage.setItem("loginsal",JSON.stringify(obj));
              navigate("/SalonHomePage")
          }
        }
      }
    });
